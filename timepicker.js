...
(function($){
  $.fn.timePicker = function(options) {
    if (options == "refreshPosition") {
      // refresh the position of the timepicker - incase anything was added to the body via other javascript or other plugins  
      var elmOffset = $(this).offset();
      var $tpDiv    = $("#time-picker-"+$(this).attr("id"));      
      $tpDiv.css({'top':elmOffset.top, 'left':elmOffset.left}).hide();
    } else {
  ...
  $._timePicker = function(elm, settings) {
    ...
    var $tpDiv = $('<div id="time-picker-'+elm_id+'" class="time-picker'+ (settings.show24Hours ? '' : ' time-picker-12hours') +'"></div>');
    var $tpList = $('<ul></ul>');
